...
public class AuthController {
    ...
    @Operation(summary = "User Registration", 
               description = "Register new user (merchant or customer). For customers: merchantId > 0, For merchants: merchantId = 0")
    public ResponseEntity<Map<String, Object>> register(@Valid @RequestBody RegisterRequest req) {
        try {
            if (req.getMerchantId() == null) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(ResponseBuilder.error(400, "MerchantId is required (0 for merchant, >0 for customer)"));
            }
            
            AuthResponse authResponse = auth.registerUser(req);
            String userType = req.getMerchantId() == 0 ? "merchant" : "customer";
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(ResponseBuilder.success(201, userType + " registration successful", authResponse));

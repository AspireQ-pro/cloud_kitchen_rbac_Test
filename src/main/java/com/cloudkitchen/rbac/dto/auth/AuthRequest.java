...
public class AuthRequest {
    @Schema(description = "Merchant ID (0=merchant/admin login, >0=customer login)", example = "0", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "MerchantId is required")
    @Min(value = 0, message = "Merchant ID must be 0 or positive")
    private Integer merchantId;

    @Schema(description = "Username for merchant/admin (merchantId=0) or phone for customer (merchantId>0)", example = "yogesh_kitchen", requiredMode = Schema.RequiredMode.REQUIRED)
